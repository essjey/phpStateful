        <nav class="bg-gray-800/50">
                    ...
                    <div class="flex items-center">
                        ...
                        <div class="hidden md:block">
                            <div class="ml-0 flex items-baseline space-x-4">
                                ...
                                <a href="/" aria-current="page" class="<?= urlIs('/') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> rounded-md px-3 py-2 text-sm font-medium text-white">Home</a>
                                <a href="/about" class="<?= urlIs('/about') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">About</a>
                                <?php if ($_SESSION['user'] ?? false) : ?>
                                    <a href="/notes" class="<?= urlIs('/notes') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Notes</a>
                                <?php endif; ?>
                                <a href="/contact" class="<?= urlIs('/contact') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> rounded-md px-3 py-2 text-sm font-medium text-gray-300 hover:bg-white/5 hover:text-white">Contact</a>
                            </div>
                        </div>
                    </div>
                    <div class="hidden md:block">
                                ...
                                <el-dropdown class="relative ml-3">
                                    ...
                                    <el-menu anchor="bottom end" popover class="w-48 origin-top-right rounded-md bg-gray-800 py-1 outline-1 -outline-offset-1 outline-white/10 transition transition-discrete [--anchor-gap:--spacing(2)] data-closed:scale-95 data-closed:transform data-closed:opacity-0 data-enter:duration-100 data-enter:ease-out data-leave:duration-75 data-leave:ease-in">
                                        ...
                                        <form action="/session" method="POST">
                                            ...
                                            <button type="submit" class="w-full text-left block px-4 py-2 text-sm text-gray-300 focus:bg-white/5 focus:outline-hidden">Sign out</button>
                                        </form>
            ...
            <el-disclosure id="mobile-menu" hidden class="block md:hidden">
                ...
                <div class="border-t border-white/10 pt-4 pb-3">
                    ...
                    <?php else : ?>
                        <div class="ml-3">
                            <a href="/register" class="<?= urlIs('/register') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> block rounded-md px-3 py-2 text-base font-medium">Register</a>
                            <a href="/login" class="<?= urlIs('/login') ? "bg-gray-950/50 text-white" : "text-gray-300 hover:bg-white/5 hover:text-white"; ?> block rounded-md px-3 py-2 text-base font-medium">Login</a>
                        </div>
                    <?php endif; ?>
                </div>
            </el-disclosure>
